...
if (system.args.length === 1) {
    console.log('Usage: collect_included_url.js <some URL> <dedicated directory>');
    phantom.exit(1);
}
address = system.args[1];
dump_dir = system.args[2];



page.settings.resourceTimeout = 15 * 1000; // express in milliseconds
...
page.onResourceError = function(resourceError) {
    ...
    console.log("onResourceError => " + page.reason + " from " + page.reason_url);
};
...
page.onUrlChanged = function (targetUrl) {
    ...
    try {
        fs.write(system.args[2] + "/__events", "OnUrlChanged " + targetUrl + "\n", 'a+');
    } catch(e) {
        console.log(e);
    }
...
page.onResourceRequested = function ( requestData, networkRequest) {
    ...
    // console.log(JSON.stringify(requestData.headers, ["Referer"], 3));
    console.log(JSON.stringify(requestData.headers, null, 3));
...
page.onResourceReceived = function (res) {
    // this is the function usable to dump:
    // content type, cookie, headers and URL received
    ...
    var info = { 'id': res.id, 'url': res.url };
    // console.log(JSON.stringify(res.headers, ["Content-type", "name"], "\t"));

    for(a in res.headers)
    {
        if (res.headers[a].name == 'Set-Cookie') {
            info.cookie = res.headers[a].value
        }
    }

    /* text/javascript application/x-javascript and application/javascript */
    if(res.contentType != null && res.contentType.indexOf('javascript') != -1) {
        fs.write(dump_dir + "/_"  + res.id + ".js", res);
    }

    // console.log(getPackages());
    // console.log(page.getWindow);

    try {
        fs.write(dump_dir + "_i" + res.id, JSON.stringify(info), 'w+');
        fs.write(dump_dir + "_A" + res.id, JSON.stringify(res), 'w+');
        fs.write(dump_dir + "_colly", JSON.stringify(info, undefined, 4), 'a+');
    } catch(e) {
       console.log(e);
    }
};
...
// This is the first executed command over 'page' object
page.open(address, function (status) {


    if (status !== 'success') {
        console.log('FAIL to load the address: ' + address + ' ' + status);
        console.log('from onResourceError ' + page.reason);
        console.log('from onResourceErrrr ' + page.reason_url);
    } else {
        ...
        console.log(address + ' has title: ' + landing_title);
        fs.write(dump_dir + "/__title", landing_title, 'w+');
        ...
        // other page analysis I can perform here
    }
    phantom.exit();
});
